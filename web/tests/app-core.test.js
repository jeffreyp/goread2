...
describe('GoRead2 Core Frontend Functionality', () => {
    ...
    describe('Font Choice Integration', () => {
        test('should have font toggle button in DOM structure', () => {
            const fontToggleBtn = document.getElementById('font-toggle-btn');
            expect(fontToggleBtn).toBeTruthy();
            expect(fontToggleBtn.tagName).toBe('BUTTON');
            expect(fontToggleBtn.title).toContain('Toggle font style');
        });

        test('should apply CSS classes correctly for font switching', () => {
            // Test initial state
            expect(document.body.classList.contains('font-serif')).toBe(false);

            // Test adding serif class
            document.body.classList.add('font-serif');
            expect(document.body.classList.contains('font-serif')).toBe(true);

            // Test removing serif class
            document.body.classList.remove('font-serif');
            expect(document.body.classList.contains('font-serif')).toBe(false);
        });

        test('should handle font button click events', () => {
            const fontToggleBtn = document.getElementById('font-toggle-btn');
        ...
        test('should handle keyboard shortcuts without interfering with existing shortcuts', () => {
            let fKeyHandled = false;
            let jKeyHandled = false;

            // Add test handlers
            document.addEventListener('keydown', (e) => {
                if (e.key === 'f') fKeyHandled = true;
                if (e.key === 'j') jKeyHandled = true;
            });
            ...
            expect(fKeyHandled).toBe(true);

            // Test j key (existing shortcut)
            const jKeyEvent = new KeyboardEvent('keydown', {
                key: 'j',
                bubbles: true
            });
            document.dispatchEvent(jKeyEvent);
            expect(jKeyHandled).toBe(true);
        });

        test('should preserve article content structure when font changes', () => {
            ...
            articleContent.innerHTML = `
                ...
                <div class="content">
                    <p>Test article content paragraph.</p>
                    <ul>
                        <li>List item 1</li>
                        <li>List item 2</li>
                    </ul>
            ...
            // Verify structure before font change
            expect(articleContent.querySelector('h1')).toBeTruthy();
            expect(articleContent.querySelector('.meta')).toBeTruthy();
            expect(articleContent.querySelector('.content')).toBeTruthy();
            expect(articleContent.querySelectorAll('li')).toHaveLength(2);

            // Simulate font change by adding class
            document.body.classList.add('font-serif');

            // Verify structure is preserved after font change
            expect(articleContent.querySelector('h1')).toBeTruthy();
            expect(articleContent.querySelector('.meta')).toBeTruthy();
            expect(articleContent.querySelector('.content')).toBeTruthy();
            expect(articleContent.querySelectorAll('li')).toHaveLength(2);
            expect(articleContent.querySelector('h1').textContent).toBe('Test Article Title');

            // Cleanup
            document.body.removeChild(articleContent);
        });

        test('should not interfere with feed list functionality', () => {
            const feedList = document.getElementById('feed-list');

            // Add test feed
            const feedItem = document.createElement('div');
            feedItem.className = 'feed-item';
            feedItem.dataset.feedId = 'test-feed';
            feedItem.innerHTML = `
                <span class="feed-title">Test Feed</span>
                <span class="unread-count" data-count="5">5</span>
            `;
            feedList.appendChild(feedItem);
            ...
            // Verify feed functionality is preserved
            expect(feedItem.querySelector('.feed-title').textContent).toBe('Test Feed');
            expect(feedItem.querySelector('.unread-count').textContent).toBe('5');
            expect(feedItem.dataset.feedId).toBe('test-feed');
        ...
        test('should work with modal dialogs', () => {
            ...
            // Apply font change while modal is open
            document.body.classList.add('font-serif');
