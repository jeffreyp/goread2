class GoReadApp {
    ...
    async init() {
        ...
        if (this.user) {
            ...
            console.log('About to start Promise.all...');
            const subscriptionPromise = this.loadSubscriptionInfo();
            const feedsPromise = this.loadFeedsOptimized();
            console.log('Created promises, starting Promise.all...');
            
            Promise.all([subscriptionPromise, feedsPromise]).then(() => {
    ...
    async loadFeedsOptimized() {
        console.log('loadFeedsOptimized function called!');
        try {
            console.log('Starting loadFeedsOptimized...');
            // Show loading state - only replace non-special feed items
            const feedList = document.getElementById('feed-list');
            console.log('loadFeedsOptimized: feedList element:', feedList);
            if (!feedList) {
                console.error('ERROR: feed-list element not found in loadFeedsOptimized!');
                throw new Error('feed-list element not found');
            }
            const nonSpecialFeeds = feedList.querySelectorAll('.feed-item:not(.special)');
            console.log('loadFeedsOptimized: removing', nonSpecialFeeds.length, 'non-special feeds');
            nonSpecialFeeds.forEach(item => item.remove());
            
            const loadingDiv = document.createElement('div');
            loadingDiv.className = 'loading';
            loadingDiv.textContent = 'Loading feeds...';
            feedList.appendChild(loadingDiv);
            console.log('loadFeedsOptimized: loading div added, feedList innerHTML:', feedList.innerHTML);
            
            document.getElementById('article-list').innerHTML = '<div class="loading">Loading articles...</div>';
            
            console.log('Making API requests for feeds and unread counts...');
            // Batch feeds and unread counts requests
            const [feedsResponse, countsResponse] = await Promise.all([
                fetch('/api/feeds'),
                fetch('/api/feeds/unread-counts')
            ...
            console.log('API responses received:', {
                feedsStatus: feedsResponse.status,
                countsStatus: countsResponse.status
            });
            ...
            this.feeds = feedsData;
            console.log('this.feeds set to:', this.feeds);
            
            if (Array.isArray(this.feeds)) {
                console.log('Feeds is array, rendering feeds...');
                this.renderFeeds();
                ...
                if (this.feeds.length > 0) {
                    ...
                    const allFeedElement = document.querySelector(`[data-feed-id="all"]`);
                    if (allFeedElement) {
                        allFeedElement.classList.add('active');
                    }
            ...
            console.error('Failed to load feeds in loadFeedsOptimized:', error);
            this.showError('Failed to load feeds: ' + error.message);
        }
    }

    async loadFeedsOptimizedWithCacheBust() {
        console.log('loadFeedsOptimizedWithCacheBust called - bypassing cache');
        try {
            console.log('Starting loadFeedsOptimized with cache bust...');
            // Show loading state - only replace non-special feed items
            const feedList = document.getElementById('feed-list');
            console.log('loadFeedsOptimized: feedList element:', feedList);
            if (!feedList) {
            ...
            console.log('Making API requests for feeds and unread counts with cache bust...');
            // Add cache-busting timestamp to force fresh data
            const timestamp = Date.now();
            const [feedsResponse, countsResponse] = await Promise.all([
                fetch(`/api/feeds?t=${timestamp}`),
                fetch(`/api/feeds/unread-counts?t=${timestamp}`)
            ]);
            ...
            const feedsData = await feedsResponse.json();
            console.log('Feeds data received (cache-busted):', feedsData);
            this.feeds = feedsData;
            console.log('this.feeds set to:', this.feeds);
            ...
            console.error('Failed to load feeds in loadFeedsOptimizedWithCacheBust:', error);
            this.showError('Failed to load feeds: ' + error.message);
        }
    }

    renderFeeds() {
        console.log('renderFeeds called with feeds:', this.feeds);
        if (!Array.isArray(this.feeds)) {
            console.log('Feeds is not an array, returning');
            return;
        }
        
        console.log('Getting feed-list element...');
        const feedList = document.getElementById('feed-list');
        console.log('feedList element:', feedList);
        console.log('feedList innerHTML:', feedList ? feedList.innerHTML : 'feedList is null');
        
        if (!feedList) {
            console.error('ERROR: feed-list element not found in DOM!');
            console.log('All elements with class feed-list:', document.querySelectorAll('.feed-list'));
            return;
        }
        
        // Remove existing feed items and loading div (not the "All" item)
        console.log('Removing existing feed items and loading div...');
        const existingFeeds = feedList.querySelectorAll('.feed-item:not(.special)');
        const loadingDiv = feedList.querySelector('.loading');
        console.log('Existing feeds to remove:', existingFeeds.length);
        console.log('Loading div to remove:', loadingDiv);
        existingFeeds.forEach(item => item.remove());
        if (loadingDiv) {
            loadingDiv.remove();
        }
        console.log('Existing feeds and loading div removed');
        
        console.log('Starting to render individual feeds...');
        this.feeds.forEach((feed, index) => {
            console.log(`Rendering feed ${index}:`, feed);
            ...
            deleteButton.addEventListener('click', (e) => {
                console.log('Delete button clicked for feed:', feed.id, feed.title);
                e.stopPropagation();
                this.deleteFeed(feed.id);
            });
            
            feedList.appendChild(feedItem);
            console.log(`Feed ${index} rendered and appended to DOM`);
        });
        console.log('All feeds rendered successfully');
    }
    ...
    async selectFeed(feedId) {
        ...
        if (feedElement) {
            ...
            console.warn(`Feed element not found for feedId: ${feedId}`);
        }
    ...
    applyUnreadCounts(unreadCounts) {
        ...
        console.log('applyUnreadCounts: Looking for all-unread-count element...');
        const allUnreadElement = document.getElementById('all-unread-count');
        console.log('applyUnreadCounts: allUnreadElement found:', allUnreadElement);
        if (allUnreadElement) {
            allUnreadElement.textContent = totalUnread;
            allUnreadElement.dataset.count = totalUnread;
            console.log('applyUnreadCounts: Updated all-unread-count to', totalUnread);
        } else {
            console.warn('All unread count element not found');
            console.log('applyUnreadCounts: All elements with id:', document.querySelectorAll('[id*="unread"]'));
        }
    ...
    async deleteFeed(feedId) {
        console.log('deleteFeed called with feedId:', feedId);
        if (!confirm('Are you sure you want to remove this feed from your subscriptions?')) {
            console.log('User cancelled deletion');
            return;
        }
        
        console.log('User confirmed deletion, making DELETE request...');
        try {
            ...
            console.log('Feed deleted successfully, cleaning up...');
            // Always clear current selection after delete
            this.currentFeed = null;
            this.currentArticle = null;
            this.articles = [];
            
            console.log('Reloading feeds and subscription info...');
            // Force refresh feeds by bypassing cache after deletion
            await this.loadFeedsOptimizedWithCacheBust();
            await this.loadSubscriptionInfo();
            this.updateSubscriptionDisplay();
            
            console.log('Selecting "all" feed after deletion...');
            // Always go to "Articles" after delete
            this.selectFeed('all');
            console.log('Delete feed process completed successfully');
        } catch (error) {
