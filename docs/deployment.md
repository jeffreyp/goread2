...
- **Cleanup**: after deploying, every other `staging-*` version is deleted, keeping only the one just deployed; `staging-*` versions would otherwise accumulate one per push indefinitely. Restricted to versions with `traffic_split=0`: an older `staging-<sha>` can be the version currently serving 100% of production traffic, since `deploy-prod.yml` (below) promotes by migrating traffic directly onto a `staging-<sha>` version rather than redeploying under a new name. This guard is what stops the very next push from deleting live production.
